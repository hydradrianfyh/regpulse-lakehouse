...
import { triggerScan, getRunLogs, getRunDocuments } from '../lib/api-client';
...
export function RunMonitor({ runs, apiConfigured, onAfterRun }: RunMonitorProps) {
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      ...
      try {
        const res = await getRunDocuments(activeRunId);
        if (cancelled) return;
        setDocuments(res.documents as RunDocument[]);
      } catch (err: any) {
        if (cancelled) return;
        setDocumentsError(err?.message || '文档加载失败');
      } finally {
    ...
    loadDocuments();
    docTimer = window.setInterval(loadDocuments, 5000);
  ...
  const getStageColor = (stage: ProgressMessage['stage']) => {
    switch (stage) {
      ...
      case 'download': return 'text-blue-700';
      case 'ingest': return 'text-emerald-700';
      ...
      <div className="rounded-xl border border-slate-200 bg-white">
        <div className="border-b border-slate-200 px-6 py-4">
          <h3 className="flex items-center gap-2 text-lg font-semibold text-slate-900">
            <FileStackIcon className="h-5 w-5 text-slate-400" />
            采集文档卡片
          </h3>
          <p className="mt-1 text-sm text-slate-500">每个卡片代表一份原始文件，可直接下载</p>
        </div>
        ...
        <div className="p-6">
          {documentsLoading && (
            <div className="text-sm text-slate-400">文档加载中...</div>
          )}
          {!documentsLoading && documents.length === 0 && (
            <div className="text-sm text-slate-500">暂无文档，请先运行采集</div>
          )}
...
function DocumentCard({ doc }: { doc: RunDocument }) {
  const meta = doc.meta || {};
  const files = normalizeFiles(meta, doc.url);
  const relevant = Array.isArray(meta.relevant_to) ? meta.relevant_to : [];
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h4 className="mt-1 font-semibold text-slate-900 break-words">{doc.title || 'Untitled'}</h4>
          {meta.document_type && (
            <div className="mt-1 text-xs text-indigo-600">类型: {meta.document_type}</div>
          )}
        </div>
        <div className="text-xs text-slate-500">
          {meta.document_date || new Date(doc.retrieved_at).toLocaleDateString('zh-CN')}
        </div>
      </div>

      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">
        <div>Reference: {meta.reference_number || '-'}</div>
        <div>Submitted by: {meta.submitted_by || '-'}</div>
        <div>Meeting: {meta.meeting_sessions || '-'}</div>
        <div>Relevant to: {relevant.length ? relevant.join(', ') : '-'}</div>
      </div>

      <div className="mt-3 border-t border-slate-100 pt-3">
        <div className="text-xs font-medium text-slate-700">Download from UNECE</div>
        <div className="mt-2 flex flex-wrap gap-2">
          {files.length === 0 && (
            <span className="text-xs text-slate-400">暂无可下载文件</span>
          )}
          {files.map((file, idx) => (
            <a
              key={`${file.url || file.download_url}-${idx}`}
              href={file.download_url ? `${API_BASE}${file.download_url}` : file.url}
              target="_blank"
              rel="noreferrer"
            ...
            >
              ...
              {file.label || file.ext?.toUpperCase() || '下载'}
              {file.source === 'UNECE' && (
                <span className="rounded bg-amber-50 px-1 text-[10px] text-amber-700">UNECE</span>
              )}
              {file.source === 'GAR' && (
                <span className="rounded bg-emerald-50 px-1 text-[10px] text-emerald-700">GAR</span>
              )}
            </a>
          ))}
        </div>
...
function normalizeFiles(meta: Record<string, any>, fallbackUrl: string) {
  const files: Array<{
    url?: string;
    download_url?: string;
    label?: string;
  ...
  if (Array.isArray(meta.stored_files)) {
    for (const file of meta.stored_files) {
      if (file?.download_url || file?.url) files.push(file);
    }
  }

  if (files.length === 0 && Array.isArray(meta.raw_files)) {
    for (const file of meta.raw_files) {
      if (file?.url) files.push(file);
    }
  }

  if (meta.raw_file_uri && !files.find((f) => f.url === meta.raw_file_uri)) {
    files.push({ url: meta.raw_file_uri });
  }

  if (files.length === 0 && fallbackUrl) {
    files.push({ url: fallbackUrl });
  }
