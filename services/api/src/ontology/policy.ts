...
export interface PolicyProfile {
  id: string;
  connector: "eu_news" | "globalautoregs";
  domain: string;
  path: string;
...
interface PolicyConfig {
  ...
  tiers: Record<string, { description?: string; allow_auto_merge?: boolean; route?: string; domains?: string[] }>;
...
export async function evaluateSource(url: string): Promise<SourceEvaluation> {
  ...
  if (profile) {
    const route = profile.tier === "TIER_A_BINDING" && !profile.requires_review ? "main" : "review_queue";
    return {
      tier: profile.tier,
      stage: profile.stage,
  ...
  if (tierFallback) {
    const route = tierFallback === "TIER_A_BINDING" ? "main" : "review_queue";
    return {
      tier: tierFallback,
      stage: "Official",
